...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="egg"; }
echo 'egg() { "$HOME/core-token/packages/core/bin/run" "$@" --token="egg"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/cambo/egg_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/cambo/egg_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@egg"
rm -rf "$HOME/.config/@Egg"
rm -rf "$HOME/.config/egg-core"
